 ...
 * - TC: O(N!) or O(N^N), (O(N^N) as for every queen we have n choice regardless of safe position)
 *  - TC explaination:
...
public class NQueens {
    ...
    static int placeQueens(boolean[][] board, int row) {
        ...
        for (int col = 0; col < board.length; col++) {
            // checking if it is safe or not to place the queen at the current (row, col)
            // position
    ...
    static boolean isSafe(boolean[][] board, int row, int col) {
        /*
         * To check wheter the curret (r,c) place is safe to place a queen or not, we
         * have to check in three directions or condition:
         * 
        ...
        for (int i = 0; i < row; i++) {
            ...
            if (board[i][col]) {
                // if true, it means there is already a queen placed, so it is not possible to
                // place queen at this row
                return false;
            }
        }
        ...
        for (int i = 1; i <= left; i++) {
            // checking for each diagonal at left side, if it is possible to place a new
            // queen or not
            if (board[row - i][col - i]) {
                // if this condition is true, it means there is already a queen up there in left
                // diagonal, so we can't place a new queen at curr position
                return false;
            }
        }

        // right diagonal
        int right = Math.min(row, board.length - col - 1); // we have to goo till the min of the elements at left side
                                                           // of current position and element at the board.length - col
                                                           // - i which is right side of the current position
        for (int i = 1; i <= right; i++) {
            // checking for each diagonal at right side, if it is possible to place a new
            // queen or not
            if (board[row - i][col + i]) {
         ...
         * 3. Right Diagonal
         * 
         * are false it means it is possible to place our queen at the current (row,
         * col) position,
